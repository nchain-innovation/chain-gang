...
impl PyStack {
    ...
    fn decode_element(&mut self, py: Python<'_>, index: Option<usize>) -> PyResult<Py<PyLong>> {
        ...
        println!("decode_element {}", result_str);
    ...
    fn single_from_array_nums(_py: Python, array: &Bound<'_, PyList>) -> PyResult<Self> {
        ...
        for item in array.iter() {
            ...
            //Convert BigInt to a Vec<u8> in little-endian format
            //let bytes = big_int.to_bytes_le();
            //inner_stack.push(py_long);
            // Convert PyLong to a string for parsing to BigInt
            //let big_int_str = py_long.str()?.to_str()?.to_owned();
            //let big_int = BigInt::from_str(&big_int_str)
            //    .map_err(|_| pyo3::exceptions::PyValueError::new_err("Failed to parse BigInt"))?;

            //inner_stack.push(big_int);
            // Each item should be a Python bytes object, which we convert to Vec<u8>
            //let bytes: &PyBytes = item.extract()?;
            //inner_stack.push(bytes.as_bytes().to_vec());
            //println!("Extracting in from_array");
            //let int_val: i64 = item.extract()?;  // Assuming we're dealing with i64 values
            //println!("Extracted in {}", int_val);
            // Convert integer to bytes in little-endian format
            //let bytes = int_val.to_le_bytes().to_vec();
            //inner_stack.push(bytes);
        }
        Ok(PyStack {
    ...
    fn single_from_array_bytes(_py: Python, array: &Bound<'_, PyList>) -> PyResult<Self> {
        ...
        /*
                for item in array.iter(){
                    let val = if let Ok(byte_val) = item.extract::<u8>() {
                        vec![byte_val] // If it fits in u8, make it a single byte
                    } else {
                        // If it's larger, convert it to bytes
                        let py_long = item.downcast::<PyLong>()?;
                        py_long
                            .to_bytes(PyBigEndian)? // Adjust to the desired byte order (e.g., little endian)
                            .to_vec() // Get the vector of bytes

                    };
                    inner_val.extend(val);
                }
        */
