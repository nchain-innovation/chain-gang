use crate::{
    ...
    util::ChainGangError,
    wallet::{
        bip32_path, bip44_path, derive_extended_key, load_wordlist, mnemonic_to_seed,
        watch_bip32_path, watch_bip44_path, ExtendedKey, HdWallet, HdWatchWallet, Wordlist,
        BSV_COIN_TYPE,
    },
};
...
impl PyHdWallet {
    ...
    fn address_at_bip44(
        ...
        Ok(self
            .inner
            .address_at_bip44(coin_type, account, external, index)?)
    }
...
fn parse_network(network: &str) -> PyResult<Network> {
    str_to_network(network)
        .ok_or_else(|| ChainGangError::BadArgument(format!("Unknown network {}", network)).into())
}
