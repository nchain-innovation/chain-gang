...
// --------------------------------------------------------------------------------------------
// Name <-> byte lookup tables
// --------------------------------------------------------------------------------------------

/// Generates opcode name/byte lookup tables from lists of opcode identifiers.
///
/// Names are produced from the identifiers via `stringify!` and byte values come from the
/// `OP_*` constants above, so a name can never drift from its value. `canonical` opcodes
/// appear in both the byte->name and name->byte directions; `name_only` opcodes (aliases
/// such as `OP_TRUE`, and names the string parser accepts but never emits, such as the
/// push-data opcodes) appear only in the name->byte direction.
macro_rules! opcode_lookup_tables {
    (
        canonical: [$($c:ident),* $(,)?],
        name_only: [$($n:ident),* $(,)?] $(,)?
    ) => {
        /// Returns the canonical name for a known opcode byte (not a direct push), if any.
        ...
        /// Every `(name, byte)` pair the script-string parser accepts, canonical names first.
        pub static OPCODE_NAME_TO_BYTE: &[(&str, u8)] = &[
            $( (stringify!($c), $c), )*
            $( (stringify!($n), $n), )*
        ];

        /// Returns the byte value for an opcode name (canonical or alias), if known.
...
opcode_lookup_tables! {
    canonical: [
        OP_0, OP_1NEGATE, OP_1, OP_2, OP_3, OP_4,
        OP_5, OP_6, OP_7, OP_8, OP_9, OP_10,
        OP_11, OP_12, OP_13, OP_14, OP_15, OP_16,
        OP_NOP, OP_VER, OP_IF, OP_NOTIF, OP_VERIF, OP_VERNOTIF,
        OP_ELSE, OP_ENDIF, OP_VERIFY, OP_RETURN, OP_TOALTSTACK, OP_FROMALTSTACK,
        OP_IFDUP, OP_DEPTH, OP_DROP, OP_DUP, OP_NIP, OP_OVER,
        OP_PICK, OP_ROLL, OP_ROT, OP_SWAP, OP_TUCK, OP_2DROP,
        OP_2DUP, OP_3DUP, OP_2OVER, OP_2ROT, OP_2SWAP, OP_CAT,
        OP_SPLIT, OP_SUBSTR, OP_LEFT, OP_RIGHT, OP_SIZE, OP_AND,
        OP_OR, OP_XOR, OP_EQUAL, OP_EQUALVERIFY, OP_1ADD, OP_1SUB,
        OP_NEGATE, OP_ABS, OP_NOT, OP_0NOTEQUAL, OP_ADD, OP_SUB,
        OP_MUL, OP_2MUL, OP_DIV, OP_2DIV, OP_MOD, OP_LSHIFT,
        OP_RSHIFT, OP_LSHIFTNUM, OP_RSHIFTNUM, OP_BOOLAND, OP_BOOLOR, OP_NUMEQUAL,
        OP_NUMEQUALVERIFY, OP_NUMNOTEQUAL, OP_LESSTHAN, OP_GREATERTHAN, OP_LESSTHANOREQUAL, OP_GREATERTHANOREQUAL,
        OP_MIN, OP_MAX, OP_WITHIN, OP_NUM2BIN, OP_BIN2NUM, OP_RIPEMD160,
        OP_SHA1, OP_SHA256, OP_HASH160, OP_HASH256, OP_CODESEPARATOR, OP_CHECKSIG,
        OP_CHECKSIGVERIFY, OP_CHECKMULTISIG, OP_CHECKMULTISIGVERIFY, OP_CHECKLOCKTIMEVERIFY, OP_CHECKSEQUENCEVERIFY,
    ],
    name_only: [
        OP_FALSE, OP_PUSHDATA1, OP_PUSHDATA2, OP_PUSHDATA4, OP_TRUE, OP_INVERT,
    ],
}
