...
* v0.11.0 - Added `RpcInterface`, a native Rust JSON-RPC client for a bitcoind/BSV node (implements the shared `BlockchainInterface` trait plus read-only query helpers), so Rust clients no longer need the Python `RPCInterface`; hand-rolled JSON-RPC 1.0 over `reqwest` with HTTP Basic auth and bounded configurable retries. See [RPC-Interface.md](RPC-Interface.md)
