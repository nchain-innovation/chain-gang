...
from tx_engine import Tx, TxIn, Script, sig_hash_preimage, sig_hash, SIGHASH, Context
...
def pushtx(sighash, load: bool, clean_stack: bool, sign_code: bool) -> Script:
    ...
    # compute h + G_x % GROUP_ORDER_INT
    out += Script.parse_string('OP_FROMALTSTACK OP_ADD OP_FROMALTSTACK OP_MOD')
    # Altstack required: [GROUP_ORDER_INT]
    # After this, the stack is: s (canonical form)
    # bring h + aG_x to canonical form (< GROUP_ORDER_INT / 2)
    out += Script.parse_string('OP_DUP OP_FROMALTSTACK OP_TUCK OP_2 OP_DIV OP_GREATERTHAN OP_IF OP_SWAP OP_SUB OP_ELSE OP_DROP OP_ENDIF')

    # After this, the stack is: s (big endian)
    out += Script.parse_string('OP_SIZE OP_SWAP OP_FROMALTSTACK OP_CAT 32 OP_SPLIT OP_DROP')
